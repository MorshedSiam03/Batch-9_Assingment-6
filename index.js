const toggleLoadingSpinner = (isLoading) => {
      ...
      .then((data) => {
        ...
        cardContainer.textContent = ''
          ...
          data.posts.map(post => {
              ...
              div.classList.add('mt-5','bg-[#F3F3F5]','card', 'card-side', 'bg-base-100', 'shadow-xl', 'p-5');
              div.innerHTML = `
              ...
              '</div>' 
                 ...
                 <div class="card-body mt-[-30px] ml-[-10px]">
                        <div class='flex'>
                          <p># ${post?.category}</p>
  ...
  const handleGetPostData = (title, view) => {
    ...
    div.classList.add('ms-4','w-80%','flex','m-3','bg-white','p-3', 'rounded-xl','justify-center','items-center')
    div.innerHTML = `
      <h2 class="text-xl font-[650px] me-8 md:me-28">${title}</h2>
      <div class=' flex justify-between'>
        <i class="fa-regular fa-eye"></i><p class='ms-1 mt-[-5px] text-[16px]'>${view}</p>
  ...
  const displayLatestData= (news) => {
    ...
    news.map(newsItem => {
      const div = document.createElement('div')
      div.classList.add('card', 'card-compact', 'w-[400px]', 'bg-base-100', 'shadow-xl')
      div.innerHTML = `
